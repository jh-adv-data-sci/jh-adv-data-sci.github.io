...
subtitle: "How to Make Dynamic Figures"
author: "08-Data Visualization"
format:
  html:
    toc: true
    toc-depth: 2
...
- **With answers (default)**: `quarto render exercises/data_viz.qmd`
- **Without answers** (figures only): `quarto render exercises/data_viz.qmd -P show_answers:false`
:::

::: callout-important
## Working with agentic AI

An agent can help you explore an API, diagnose an error, or propose variants. It cannot make the visual argument for you. For every redesign prompt below, state the audience and decision the figure supports; keep an AI-use log (prompt, output used, and changes you made); and verify every displayed encoding against the data. Do not submit a plot you cannot explain layer by layer.
:::
...
# 1.1) Redesign for a decision

Choose a nontechnical buyer deciding whether a diamond is in a plausible price range. Redesign the plot for that decision, then write 100–150 words explaining what you changed, what trade-off it creates, and why the density overlay is or is not still useful. An agent may propose options, but you must justify the final choice.
--- 
...
# 2.1) Audit a misleading default

Create a deliberately less informative version of this plot (for example, by changing scales, overplotting, or grouping), then repair it. Include a short caption identifying the specific inference that the bad version could encourage.
--- 
...
# 8) Mini data-story: critique, build, and verify

Choose one dataset used above or a small, public dataset relevant to your interests. Pose one answerable descriptive question for a named audience. Create a two-figure data story: one overview and one supporting or qualifying view. Each figure needs an informative title, a caption that states the takeaway, and accessible color/label choices.

Use an AI agent for at least one bounded task (such as proposing a chart type, checking a `ggplot2` API detail, or reviewing labels). Submit the prompt and a brief verification note. Your final write-up (250–400 words) must explain why each encoding is appropriate, name one limitation, and identify one change you rejected after inspecting the data. Put the analysis in a reproducible `.qmd` or `.R` file.
