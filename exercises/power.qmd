...
> **Using agentic AI:** You may use an agent to derive a formula, draft R code, or suggest a simulation. Treat it as an unverified collaborator: state your assumptions, run the code, and reconcile any disagreement with an independent calculation. For the design challenge, include the prompt(s) used and a concise verification log.
...
Two‑sided power (z‑approx): $$ 1 - \Phi(z_{0.975}-\lambda) + \Phi(-z_{0.975}-\lambda) \approx 1-\Phi(1.96-2.121)+\Phi(-1.96-2.121) \approx 1-\Phi(-0.161)+\Phi(-4.081). $$ This is approximately $1-(0.436)+\text{tiny} \approx 0.564$. Using exact **t** power:
...
**Setup.** In $Y=\beta_0+\beta_1 X+\varepsilon$ with $\varepsilon\sim N(0,\sigma^2)$, suppose $\sigma=10$ and you plan $X$ standardized (mean 0, var 1). You want a **95% prediction interval** for a *future* observation at $x_0$ to have total width $W$.
...
The **minimum achievable** half‑width as $n\to\infty$ is $t_{\infty,0.975}\sigma$, where $t_{\infty,0.975}=1.96$.
...
## 8) Design-review challenge: plan under uncertainty (20 minutes)

You are planning a two-arm study with a continuous primary outcome. A pilot suggests an SD between 10 and 14; the smallest effect worth detecting is 4 units; you can recruit at most 130 participants total; and anticipate 15% dropout. The primary analysis is a two-sided two-sample t-test at α = 0.05 with equal allocation.

**Deliverable.** Write a one-page design recommendation that:

- calculates the analyzable and enrolled sample sizes needed for 80% power at SD = 10, 12, and 14;
- reports achieved power at the maximum feasible enrollment after dropout;
- gives a transparent recommendation and one contingency if the pilot SD is too optimistic; and
- verifies one analytic result with a simulation (state the number of replicates and Monte Carlo uncertainty).

You may ask an AI agent for a first-pass calculation or code, but include your prompt, independently check one output, and explain any correction you made. The grade is based on the reasoning and audit trail, not on whether an agent produced code quickly.
