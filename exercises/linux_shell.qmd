...
> **Using an AI agent well:** You may ask an agent to explain a command, draft a first attempt, or help debug an error. Before running a command that writes, deletes, transfers, or submits work, inspect it yourself. For the challenge below, submit the prompt(s) you used, the command you ran, and a short note on what you verified or corrected.

---

**If you do not have a Linux/Unix-based Machine (aka Windows), you can go to GitHub codespaces/RStudio Terminal for one of your repositories and navigate through there.**
...
**Question.** Inspect the header, show the first five penguin species, and count the distinct island values.

<details><summary>Show solution</summary>

```bash
head -n 1 penguins.csv
cut -d',' -f1 penguins.csv | tail -n +2 | head -n 5
...
**Question.** Count penguins with a bill length of at least 50 mm. Compute the mean body mass (g), excluding missing values.

<details><summary>Show solution</summary>

```bash
# bill_length_mm is column 3
awk -F',' 'NR>1 && $3 >= 50 {c++} END{print c+0}' penguins.csv

# body_mass_g is column 6
awk -F',' 'NR>1 && $6 != "" {s+=$6; n++} END{print s/n}' penguins.csv
```
</details>
...
**Question.** Count how many data rows contain an empty field.  Assume the data elements are not quoted.
...
## 9) Save the first 20 species/island records to a file
**Question.** Write the first 20 **species and island** records (not including the header) to `sample_records.txt`.

<details><summary>Show solution</summary>

```bash
tail -n +2 penguins.csv | cut -d',' -f1,2 | head -n 20 > sample_records.txt
wc -l sample_records.txt   # should be 20
```
</details>
...
**Question.** Among *male* penguins, show counts by **species** in alphabetical order.

<details><summary>Show solution</summary>

```bash
awk -F',' 'NR>1 && $7=="male"{print $1}' penguins.csv | sort | uniq -c
```
</details>
...
**Question.** Create `analyze.sh` that prints: total rows, bill-length ≥ 50 mm rows, and mean body mass. Run it.
...
echo -n "Bill length >= 50 mm rows: "
awk -F',' 'NR>1 && $3 >= 50 {c++} END{print c+0}' "$csv"

echo -n "Mean body mass (g): "
awk -F',' 'NR>1 && $6 != "" {s+=$6; n++} END{print s/n}' "$csv"
EOF
...
echo "File: $f"
echo "Total data rows: $(stream | tail -n +2 | wc -l)"
echo "Bill length >= 50 mm rows: $(stream | awk -F',' 'NR>1 && $3>=50{c++} END{print c+0}')"
echo "Mean body mass (g): $(stream | awk -F',' 'NR>1 && $6!=""{s+=$6; n++} END{print s/n}')"
EOF
...
## 18) Agent-assisted command-line investigation


**Question.** Choose a question about the shell or this dataset that you cannot yet answer (for example: create a species-by-island table, safely find all CSVs larger than 10 MB, or identify duplicate records). Use an AI agent to propose a solution, then test it on `penguins.csv` and improve it if needed.

**Submit:** your question; the agent prompt and response link/transcript; the final command or script; its output; and 3–5 sentences explaining why it is correct, including one edge case or safety check. Share one useful correction with the class.
