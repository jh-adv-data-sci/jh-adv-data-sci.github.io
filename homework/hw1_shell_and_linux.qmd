...
Your script must use `set -euo pipefail`, accept input and output directories as arguments, be safe to re-run, and report the number of files it organized. Add a verification command that proves there are exactly 400 files in the expected subject/visit/type layout. Submit the script, a short `README.md` with the command used, and terminal output (not only screenshots) in your GitHub Classroom repository.
