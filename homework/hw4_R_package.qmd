...
## 2026 extension: trustworthy package interface

Expose a validated, user-facing function that returns both the design recommendation and its assumptions. Test invalid inputs and at least one known-answer case, not only successful runs. Add a concise changelog and a reproducible example that generates a design sensitivity table or figure. Configure CI to run `R CMD check` and tests on a clean environment.
